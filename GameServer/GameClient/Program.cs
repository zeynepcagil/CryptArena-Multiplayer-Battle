...
namespace GameClient
{
    ...
    class Program
    {
        ...
        // Son çizilen pozisyonları takip et (temizlik için)
        ...
        private static void ConnectToServer()
        {
            ...
            catch { Console.WriteLine("Sunucu kapali!"); Console.ReadKey(); }
        }
        ...
        private static void ManaRegenLoop()
        {
            while (true)
            {
                ...
                {
                    lock (_drawLock) { _myMana = Math.Min(_myMana + _manaRegenRate, _myMaxMana); }
                    DrawUI();
                }
            }
        }
        ...
        private static void ProcessMoveData(string m)
        {
            try
            {
                ...
                if (p.Length < 4 || p[0] == _myName) return;
        ...
        private static void ProcessStatusData(string m)
        {
            try
            {
                ...
                if (p.Length < 3 || p[0] == _myName) return;
                ...
                lock (_drawLock)
                {
                    ...
                    _enemies[p[0]].HP = hp;
                    _enemies[p[0]].MaxHP = maxHp;

                    if (hp <= 0 && !_enemies[p[0]].Avatar.Contains("💀") && !_enemies[p[0]].Avatar.Contains("👻"))
                        _enemies[p[0]].Avatar = "👻";
        ...
        static void DrawGame()
        {
            lock (_drawLock)
            {
                ...
                foreach (var kvp in _lastDrawnPositions.ToArray())
                {
                    ...
                    else if (_enemies.ContainsKey(kvp.Key))
                    {
                ...
                _lastDrawnPositions[_myName] = (_myX, _myY, 2);
